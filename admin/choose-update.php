...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($icon_err) && empty($title_err) && empty($content_err)){
        ...
        $sql = "UPDATE abouts SET icon=?, title=?, content=?, id_link=? WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssi", $param_icon, $param_title, $param_content,$param_id_link, $param_id);
